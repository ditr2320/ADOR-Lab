...
for(j in seq(1:length(outcomes))) 
{
    ...
    
  {
    ...
    #linear model with adjustment factors. 
      ...
      modelForm5=as.formula("thisOutcome ~ thisExposureFactor + as.factor(sex) + age + as.factor(race) + MET")
...
#write.table(LM_bmi,paste0(meta3,"hyphy_lm_bmi_noSRPA_06Oct2021.csv",sep=""),quote=FALSE, sep=",",append=FALSE, col.names=TRUE, row.names=FALSE)
#write.table(LM_whr,paste0(meta3,"hyphy_lm_whr_noSRPA_06Oct2021.csv",sep=""),quote=FALSE, sep=",",append=FALSE, col.names=TRUE, row.names=FALSE)
