...
dataSetChange <- dataSet1m %>% select(c(1:3), SES_r)
dataSetChange$timepoint <- "Change"
dataSetChange$BMI_change <- (dataSet6m$mom_BMI - dataSet1m$mom_BMI) 
dataSetChange$mother_age <- dataSetAv$mother_age
dataSetChange$m_ener_Mom <- dataSetAv$m_ener_Mom 
dataSetChange$exercise_r <- dataSetAv$exercise_r
dataSetChange$SES_r <- dataSetAv$SES_r
...
for(taxa in taxaLevels)
{
  ...
  taxaMeta <- merge(dataSetChange,microbiomeChange,by="dyad_id")
  ...
  allExposures <- names(taxaMeta)[(length(dataSetChange) + 1):(length(taxaMeta))]
  ...
  #pdf(file = paste0(meta2,"plots/microbiomeAverage_",taxa,"_",variableOfInterest,"_lm_withCovariates.pdf",sep=""))
    
  # Need to ALWAYS verify this is using the correct names based on our dataset
  for(i in (length(dataSetChange) + 1):(length(taxaMeta)))
  {
    ...
    modelForm <- as.formula(paste("thisMicrobe", "~thisVariable+age+energy+exercise+SES")) #add BMI 
  ...
  write.table(allExposures,paste0(meta2,"statisticalTables/momAverage_",taxa,"_Microbiome_lm_withCovariates.txt",sep=""),quote=FALSE, sep="\t",append=FALSE, col.names=TRUE, row.names=FALSE)
}
