...
ExposureFactors <- c("BMI_change") 

#setting up outcomes to loop through
MaternalFactors <- c("shannon_entropy","faith_pd", "gut_richness", "FBratio")
...
trimmedTable <- trimmedTable %>% rename("Exposure" = "Dietary Index","Beta" = "Beta Mod3", "P-value" = "pvalue_Mod3", "n" = "sampleSize")
...
data1[data1 == "BMI_change"] <- "Change in BMI"
data1
hux <- data1 %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.00345" = "red"))
#quick_pdf(hux)
