...
MacroBaseline<-names(dataSet)[c(512:515,506,852)]
dpBaseline<-names(dataSet)[c(893,898,896)]
HMOBaseline<-names(dataSet)[c(987,988,1010,1009)]
AnthroBaseline <-names(dataSet)[c(4,468)]
...
ExposureFactors <- MacroBaseline

#setting up outcomes to loop through
outcomes <-HMOBaseline[2:4]
...
for(j in seq(1:length(outcomes))) 
{
    ...
    
  {
    ...
    thisDataInstance <- na.omit(data.frame(thisOutcome = metadata[,names(metadata) %in% outcomes[j]],
                                           ...
                                           age = metadata$mother_age_baseline,
                                           energy = metadata$m_ener_Mom_baseline,
                                           BMI = metadata$mom_BMI_baseline, 
