...
taxaTableMom6m <- taxaTable[,grepl("Mom.6m",colnames(taxaTable))] #either chance this line or create 2d loop
...
MaternalFactors <- c("mom_BMI", "shannon_entropy","faith_pd","gut_richness", "FBratio")
...
for(j in seq(1:length(MaternalFactors)))
{
    ...
    
  {
    ...
    if(MaternalFactors[j] == "mom_BMI")
    {
      ...
      #1) Model 1 base model + Kcal
      modelForm1=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aBMI")
      #2) Model 2 base model + Kcal + Physical Activity 
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aBMI + aExercise")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aBMI + aExercise + SES_index_final")
    }
...
data1
hux <- data1 %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.00892" = "red", "0.059" = "orangered"))
#quick_pdf(hux)
