...
for(taxa in taxaLevels)
{
  taxaTable <- read.delim(paste0(meta5,"mm_150bp_deblur_sepp_noMit_noChl_rmsingletons_",taxa,".txt",sep=""),header=TRUE,row.names=1,check.names = FALSE)
    ...
    dataSetAv$dyad_id <- dataSetAv$study_id
    dataSet1m$dyad_id <- dataSet1m$study_id
  ...
  write.table(allExposures,paste0(meta2,"statisticalTables/BSI_momChange_",taxa,"_Microbiome_lm_withCovariates.txt",sep=""),quote=FALSE, sep="\t",append=FALSE, col.names=TRUE, row.names=FALSE)
}
