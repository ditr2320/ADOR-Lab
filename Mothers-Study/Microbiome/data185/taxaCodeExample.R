...
require(ggplot2);require(lmSupport);
require(gamlss);require(foreign);
...
dataSet <- read.delim(paste0(meta,"mothersMilk_metadata_timepointsAsRows_updated091620_Temporary24mDiet.txt"), header=TRUE)
...
dataSet12m <- dataSet[dataSet$timepoint %in% "12",]
dataSet24m <- dataSet[dataSet$timepoint %in% "24",]
...
for(taxa in taxaLevels)
{
  ...
  taxaMeta <- merge(dataSet1m,myT5,by="dyad_id")
  ...
  pdf(file = paste0(meta2,"plots/microbiome_",taxa,"_",variableOfInterest,"_lm_withCovariates_TEST.pdf",sep=""))
  ...
  for(i in (length(dataSet1m) + 1):(length(taxaMeta)))
  {
    ...
    modelForm <- as.formula(paste("thisMicrobe","~thisVariable+mother_age+SES")) #add BMI 
    ...
    thisDataInstance <- na.omit(data.frame(dyad_id=taxaMeta$dyad_id,
                                   ...
                                   mom_BMI=taxaMeta$mom_BMI,
                                   mother_age=taxaMeta$mother_age,
                                   SES=taxaMeta$SES_index_final))
    ...
    print(p + geom_point( size = 8,color = "grey36") +
            geom_abline(slope = coefs["thisVariable"],intercept = coefs["(Intercept)"] + 
                          mean(thisDataInstance$mother_age)*coefs["mother_age"] +
  ...
  write.table(allExposures,paste0(meta2,"statisticalTables/momBaseline_",taxa,"_Microbiome_lm_withCovariates_TEST.txt",sep=""),quote=FALSE, sep="\t",append=FALSE, col.names=TRUE, row.names=FALSE)
}
######################################################################################################################################################
######################################################################################################################################################
# Creating plots
...
print(p + geom_point( size = 8,color = "grey36") +
        geom_abline(slope = coefs["thisVariable"],intercept = coefs["(Intercept)"] + 
                      ...
                      mean(thisDataInstance$BMI)*coefs["BMI"] +
                      mean(thisDataInstance$energy)*coefs["energy"] +
                      mean(thisDataInstance$exercise)*coefs["exercise"] +
                      mean(thisDataInstance$SES)*coefs["SES"],size=1.5,linetype = "dashed",colour="grey36") +
        ggtitle(title) +
        xlab(variableOfInterest) +
        ylab(paste(names(taxaMeta)[i],sep = "")) +
