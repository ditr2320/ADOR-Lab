...
data <- dataSet[dataSet$timepoint %in% "6",]
...
ExposureNames <- c("HEI at 6-Months", "DII at 6-Months", "MDS at 6-Months")

#setting up outcomes to loop through
MaternalFactors <- c("mom_BMI") 
MaternalNames <- c("BMI at 6-Months")
...
colnames(LM_6m) <- c("Dietary Index", 
                           "Outcome",
...
LM_6m <- LM_6m %>% as_tibble()
Table <- LM_6m %>% select("Dietary Index", "Outcome", "Beta Mod3", "pvalue_Mod3", "sampleSize")
hux <- Table %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.00347" = "red"))
#quick_pdf(hux)
#write.table(LM_6m,paste0(meta2,"1856m.txt",sep=""),quote=FALSE, sep="\t",append=FALSE, col.names=TRUE, row.names=FALSE)