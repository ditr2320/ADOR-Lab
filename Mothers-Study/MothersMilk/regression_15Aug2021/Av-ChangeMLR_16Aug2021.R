...
dataSetAv$dash_score <- (dataSet6m$dash_score + dataSet1m$dash_score)/2 
...
ExposureFactors <- c("HEI2015_TOTAL_SCORE_Mom", "DII_Mom", "MDS_Mom", "dash_score") 
ExposureNames <- c("Average HEI", "Average DII", "Average MDS", "Average DASH")
...
for(j in seq(1:length(MaternalFactors)))
{
    ...
    
  {
    ...
    if(ExposureFactors[i] == "HEI2015_TOTAL_SCORE_Mom")
    {
      #0) Model 0 base model
      modelForm0=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline")
      ...
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aExercise")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aExercise + SES_index_final")
    } else {
      #0) Model 0 base model
      modelForm0=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline")
      #1) Model 1 base model + Kcal
      modelForm1=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal")
      #2) Model 2 base model + Kcal + Physical Activity 
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise + SES_index_final")
    }
    ...
    beta3Exposure1 <- format(summary(statMod3)$coefficients[2,1], digits =2) 
    pVal3Exposure1 <- format(summary(statMod3)$coefficients[2,4], digits =2) 

    #Currently plots with show beta and p value for statMod3, modify as needed
    if(summary(statMod3)$coefficients[2,4] < 0.05) 
    {
      title <- paste("beta = ",
                     format(beta3Exposure1,digits=3),
                     ", p-value = ",
                     format(pVal3Exposure1,digits=3), 
...
trimmedTable <- trimmedTable %>% rename("Average Index" = "Dietary Index","6m Outcome"="Outcome","Beta" = "Beta Mod3", "P-value" = "pvalue_Mod3", "n" = "sampleSize")
...
data1
hux <- data1 %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.00157" = "red"))
#quick_pdf(hux)
