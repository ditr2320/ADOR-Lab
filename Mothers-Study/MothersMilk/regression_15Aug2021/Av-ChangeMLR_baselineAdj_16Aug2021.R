...
ExposureNames <- c("Average HEI", "Average DII", "Average MDS")
...
for(j in seq(1:length(MaternalFactors)))
{
    ...
    
  {
    ...
    if(ExposureFactors[i] == "HEI2015_TOTAL_SCORE_Mom")
    {
      ...
      modelForm1=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + baselineBMI")
      #2) Model 2 base model + Kcal + Physical Activity 
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aExercise + baselineBMI")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aExercise + SES_index_final + baselineBMI")
    } else {
      #0) Model 0 base model
      modelForm0=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + baselineBMI")
      #1) Model 1 base model + Kcal
