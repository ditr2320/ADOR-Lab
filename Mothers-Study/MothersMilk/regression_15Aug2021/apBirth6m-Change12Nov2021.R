...
exposure <- read.delim(paste0(meta2,"apData30Nov2021.txt"), header=TRUE)
dataSet <- exposure
...
dataSetAv$TotNO2_Birth_6m_weighted <- dataSet1m$TotNO2_Birth_6m_weighted
dataSetAv$TotO3_24h_Birth_6m_weighted <- dataSet1m$TotO3_24h_Birth_6m_weighted
dataSetAv$TotO3_8h_Birth_6m_weighted <- dataSet1m$TotO3_8h_Birth_6m_weighted
dataSetAv$TotO3_1h_Birth_6m_weighted<- dataSet1m$TotO3_1h_Birth_6m_weighted
dataSetAv$TotPM25_Birth_6m_weighted<- dataSet1m$TotPM25_Birth_6m_weighted
dataSetAv$TotPM10_Birth_6m_weighted<- dataSet1m$TotPM10_Birth_6m_weighted
metadata <- dataSetAv

#TotNO2_Preg_6m_weighted	TotO3_24h_Preg_6m_weighted	TotO3_8h_Preg_6m_weighted	TotO3_1h_Preg_6m_weighted	TotPM25_Preg_6m_weighted	TotPM10_Preg_6m_weighted
#TotNO2_Birth_6m_weighted	TotO3_24h_Birth_6m_weighted	TotO3_8h_Birth_6m_weighted	TotO3_1h_Birth_6m_weighted	TotPM25_Birth_6m_weighted	TotPM10_Birth_6m_weighted
#REGRESSION LOOP
...
ExposureFactors <- c("TotNO2_Birth_6m_weighted","TotO3_24h_Birth_6m_weighted","TotO3_8h_Birth_6m_weighted","TotO3_1h_Birth_6m_weighted","TotPM25_Birth_6m_weighted","TotPM10_Birth_6m_weighted")
ExposureNames<- c("TotNO2_Birth_6m_weighted","TotO3_24h_Birth_6m_weighted","TotO3_8h_Birth_6m_weighted","TotO3_1h_Birth_6m_weighted","TotPM25_Birth_6m_weighted","TotPM10_Birth_6m_weighted")

#setting up outcomes to loop through
MaternalFactors <- c("BMI_change") 
MaternalNames <- c("BMI Change")
...
for(j in seq(1:length(MaternalFactors)))
{
    ...
    
  {
    ...
    thisDataInstance <- na.omit(data.frame(dyad_id = metadata$dyad_id,
                                           ...
                                           BMI_change = metadata$BMI_change,
                                           baselineBMI = dataSet1m$mom_BMI))
    ...
    statModForPlot <- lm(modelFormForPlot, data=metadata)
      ...
      modelForm1=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline")
      #1) Model 1 base model + Kcal
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal")
      #2) Model 2 base model + Kcal + Physical Activity 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm4=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise + SES_index_final")
    ...
    beta4Exposure <- format(summary(statMod4)$coefficients[2,1], digits = 3) 
    pVal4Exposure <- format(summary(statMod4)$coefficients[2,4], digits = 3) 
    beta4ExposureList[[length(beta4ExposureList)+1]] <- beta4Exposure
    pVal4ExposureList[[length(pVal4ExposureList)+1]] <- pVal4Exposure
    CI4ExposureLower <- format(confint(statMod4, level=0.95)[2,1], digits = 3) 
    CI4ExposureUpper <- format(confint(statMod4, level=0.95)[2,2], digits = 3) 
    ...
    if(summary(statMod4)$coefficients[2,4] < 0.5) 
    {
      title <- paste("beta = ",
                     format(beta4Exposure,digits=3),
                     ", p-value = ",
                     format(pVal4Exposure,digits=3), 
...
colnames(LMavg) <- c("Dietary Index", 
                     ...
                     "Beta Mod4","pvalue_Mod4","Lower_CI_Mod4","Upper_CI_Mod4", "sampleSize")
