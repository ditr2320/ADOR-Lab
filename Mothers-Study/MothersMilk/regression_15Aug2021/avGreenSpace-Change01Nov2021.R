...
green <- read.delim(paste0(meta2,"longGData27Oct2021.txt"), header=TRUE)
dataSet <- green
...
dataSetAv$M_100 <- (dataSet6m$M_100 + dataSet1m$M_100)/2
dataSetAv$M_300 <- (dataSet6m$M_300 + dataSet1m$M_300)/2
dataSetAv$M_500 <- (dataSet6m$M_500 + dataSet1m$M_500)/2
dataSetAv$M_1000 <- (dataSet6m$M_1000 + dataSet1m$M_1000)/2
metadata <- dataSetAv
...
ExposureFactors <- c("M_100","M_300","M_500","M_1000")
ExposureNames<- c("M_100","M_300","M_500","M_1000")
...
for(j in seq(1:length(MaternalFactors)))
{
    ...
    
  {
    ...
    statModForPlot <- lm(modelFormForPlot, data=metadata)
      ...
      modelForm0=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + baselineBMI")
      #1) Model 1 base model + Kcal
      modelForm1=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + baselineBMI")
      #2) Model 2 base model + Kcal + Physical Activity 
      modelForm2=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise + baselineBMI")
      #3) Model 3 base model + Kcal + Physical Activity + SES 
      modelForm3=as.formula("thisOutcome ~ thisExposureFactor + mother_age_baseline + aKcal + aExercise + SES_index_final + baselineBMI")
    ...
    if(summary(statMod3)$coefficients[2,4] < 0.5) 
    {
      title <- paste("beta = ",
...
data1
hux <- data1 %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.0047" = "red"))
#quick_pdf(hux)
