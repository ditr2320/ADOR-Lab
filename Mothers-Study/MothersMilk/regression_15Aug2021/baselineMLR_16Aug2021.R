...
ExposureNames <- c("HEI at 1-Month", "DII at 1-Month", "MDS at 1-Month", "DASH at 1-Month")

#setting up outcomes to loop through
MaternalFactors <- c("mom_BMI") 
MaternalNames <- c("BMI at 1-Month")
...
for(j in seq(1:length(MaternalFactors))) 
{
    ...
    
  {
    ...
    {
      title <- paste("beta = ",
                     format(beta3Exposure1),
                     ", p-value = ",
                     format(pVal3Exposure1), 
...
LM_baseline <- LM_baseline %>% as_tibble()
Table <- LM_baseline %>% select("Dietary Index", "Outcome", "Beta Mod3", "pvalue_Mod3", "sampleSize")
hux <- Table %>% as_huxtable() %>% set_all_borders(1) %>% set_bold(1, everywhere, TRUE) %>% map_text_color(by_values("0.00347" = "red"))
#quick_pdf(hux)
#write.table(LM_baseline,paste0(meta2,"185baseline.txt",sep=""),quote=FALSE, sep="\t",append=FALSE, col.names=TRUE, row.names=FALSE)
